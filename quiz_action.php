...
if(isset($_REQUEST['submit'])) {
	if($_REQUEST['action'] == 'edit') { //Update goes here
		$wpdb->get_results($wpdb->prepare("UPDATE wp_quiz_quiz SET name=%s, description=%s,final_screen=%s WHERE ID=%d", $_REQUEST['name'], $_REQUEST['description'], $_REQUEST['content'], $_REQUEST['quiz']));
		
		wp_redirect($wpframe_home . '/wp-admin/edit.php?page=quizzin/quiz.php&message=updated');
	
	} else {
		$wpdb->get_results($wpdb->prepare("INSERT INTO wp_quiz_quiz(name,description,final_screen,added_on) VALUES(%s,%s,%s,NOW())", $_REQUEST['name'], $_REQUEST['description'], $_REQUEST['content']));
